...
###############################################
my $baseDir="/users/npm65/data/MotherChild/pipeline_input";
#my $baseDir="/users/njc97/data/10Samples/variants";

my $vcf;
#my $InterestedGenefile="/users/a5907529/lustre/Kidney_20130211/john_gene_list.txt";
my $InterestedGenefile="$baseDir/john_gene_list.txt";

# Sources to add as annotation
my $ref="/users/data/reference/hsapiens/hg19/GATK_bundle/ucsc.hg19.4GATK.fasta";
my $genefile="$baseDir/Ensembl_Genes_R67.txt";
my $sharedfile="$baseDir/InHouse_OnTarget_Variants_MAFs.txt";
my $OMIMfile="$baseDir/Ensembl_OMIM_AllGeneNames.txt";
my $ens_gene_OMIM_Uniprot_Acc = "$baseDir/ens_gene_symbol_omim_id_uniprot_id.txt";
# An comlumn to tell if the variant is a SNP or INDEL
my $SNPorINDEL;

my $Results=GetOptions("vcf=s"=>\$vcf, "InterestedGenes=s"=>\$InterestedGenefile);
my $output_file = $vcf.".inHouse_annotated.vcf";
	...
	if($line =~ m/^\#/) {
		...
		if (scalar(@variants) > 1) {
			for (my $i=0;$i<scalar(@variants);$i++) {
				...
				push @output, $elements[0]."\t".$elements[1]."\t".$elements[2]."\t".$elements[3]."\t".$alter_temp."\t".$elements[5]."\t".$elements[6]."\t".$elements[7]."\t".$V_type."\t".$elements[8]."\t".$Sample_Call_processed."\t".$gene_name."\t".$ens_id."\t".$isInterested."\t".$single_maf."\t".$omim_anno."\t".$genecard_link."\t".$omim_link."\t".$uniprot_link."\n";
			}
		} else {
