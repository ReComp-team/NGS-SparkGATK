detLogLike<-function(VarFreqs,NoVarFreqs,Sens,Spez)
{
  ...
  ObsVar[ObsVar<.Machine$double.eps]<-.Machine$double.eps
  ObsNoVar[ObsNoVar<.Machine$double.eps]<-.Machine$double.eps 
  sum(log(ObsVar))+sum(log(ObsNoVar))
}

estimateShell_III<-function(Observed,Freqs,Selected.SNPs,
                            Start.Spez=0.5,Start.Sens=0.5,Eps=1e-9)
{
                          ...
                          SegmentSize=1,NBoot=1000)
{
  ...
  Results[,1]<-colMeans(Intermediate)
  Results["Sensitivity",-1]<-abc.ci(Intermediate[,1],function(x,w)weighted.mean(x, w))[-1]
  Results["Specificity",-1]<-abc.ci(Intermediate[,2],function(x,w)weighted.mean(x, w))[-1]
  Results
}
